...
3. Build the agent call with the Anthropic SDK. Give it **read-only tools**: `get_account`, `get_transaction_history`, `get_velocity_stats`. The model investigates by calling these, then returns a **structured JSON verdict**: `risk_score` (0–100), `reasoning`, `recommended_action` (APPROVE / HOLD / ESCALATE).
